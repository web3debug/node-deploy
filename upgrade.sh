...
for ((i = 0; i < ${#ips[@]}; i++)); do
    ...
    aws ssm send-command \
        ...
        --parameters commands="sudo bash /server/clusterNetwork/stop_geth.sh && echo y | sudo cp /mnt/efs/bsc-qa/bc-fusion/clusterNetwork/start_geth.sh /server/clusterNetwork/start_geth.sh"
    aws ssm send-command \
        --instance-ids "${dst_id}" \
        --document-name "AWS-RunShellScript" \
