...
ExecStart=/usr/local/bin/geth --datadir /root/.ethereum --config /root/.ethereum/config.toml --password /root/.ethereum/password.txt --blspassword /root/.ethereum/password.txt --nodekey /root/.ethereum/geth/nodekey --unlock ${cons_addr} --allow-insecure-unlock --miner.etherbase ${cons_addr} --gcmode ${gcmode} --syncmode full --mine --vote
Restart=always
RestartSec=5
